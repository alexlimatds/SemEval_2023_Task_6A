...
class SSClassifier_Cohan(torch.nn.Module):
    def __init__(self, encoder_id, n_classes, dropout_rate, sep_id, embedding_dim, use_mlp, n_hidden_mlp=100):
        ...
        such as BERT, RoBERTa, or ALBERT. The classification head can be a MLP (one 
        hidden layer) or a single layer classifier.
        Arguments:
...
class SSClassifier_Cohan_Longformer(torch.nn.Module):
    def __init__(self, encoder_id, n_classes, dropout_rate, sep_id, embedding_dim, use_mlp, n_hidden_mlp=100):
        ...
        Longformer model as encoder and a classification head. The last can be a MLP (one 
        hidden layer) or a single layer classifier.
        Arguments:
            encoder_id: ID (string) of the encoder model in Hugging Faces repository.
            n_classes: number of classes.
...
class SSClassifier_VanBerg(torch.nn.Module):
    def __init__(self, encoder_id, n_classes, dropout_rate, sep_id, embedding_dim, use_mlp, n_hidden_mlp=100):
        ...
        Arguments:
            ...
            embedding_dim: dimension of hidden units in the sentence encoder (e.g., 768 for BERT).
            use_mlp: indicates the use of a MLP classifier (True) or a single layer one (False) as classification head.
            n_hidden_mlp: the number of hidden units of the MLP classifier.
        '''
