...
class ContextualizedSC_BERT(torch.nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        for i in range(batch_size): # iterates sequences/blocks
            ...
            # we dont want the embeddings of the 1st and last CLS tokens
            idx_core_sep = idx_seps[1:-1] # index of core [SEP] in current sequence
            core_sep_emb = hidden_state[i, idx_core_sep, :] # embedding of core [SEP]
...
class LongformerEncoder(torch.nn.Module):
    '''
    An instance of this class yields sentence embeddings for a sentence chunk.
    '''
    ...
    def forward(self, input_ids, attention_mask):
        ...
        for i in range(batch_size): # iterates sequences/blocks
            ...
            for j in range(core_sep_emb.shape[0]):
                core_embeddings.append(core_sep_emb[j])
...
class DFCSC_Longformer(torch.nn.Module):
    ...
    get as inputs sentence representaions from a ContextualizedSC_Dataset object.
...
def fit(train_params, ds_train, ds_test, device):
    ...
    if use_mock:
        ...
        if encoder_id.lower().find('longformer') != -1:
