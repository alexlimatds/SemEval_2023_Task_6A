...
def evaluate_BERT(train_params):
    ...
    time_tag = f'{model_reference}_ft_{datetime.now().strftime("%Y-%m-%d-%Hh%Mmin")}'
    ...
    if train_params.get('n_documents') is not None: # used in tests to speed up the train procedure
        ...
        ds_train = models.get_dataset(temp_docs_train, labels_to_idx, tokenizer)
        ds_test = models.get_dataset(temp_docs_test, labels_to_idx, tokenizer)
    else:
        ds_train = models.get_dataset(dic_docs_train, labels_to_idx, tokenizer)
        ds_test = models.get_dataset(dic_docs_test, labels_to_idx, tokenizer)
